...
from DDPM_penalty import DDPM_penalty
from GNet_multihead_attention import GNet_multihead_attention



class DDPM_regression(nn.Module):
    def __init__(self, device, beta_1, beta_T, T, input_dim,con_dim,alpha = 0.1):
        ...
        self.ddpm = DDPM_penalty(device, beta_1, beta_T, T, input_dim,con_dim,alpha)
        self.gnet = GNet_multihead_attention(input_dim, hidden_size=32, alpha=alpha, num_heads=4)
        self.ddpm = DDPM_penalty(T, input_dim,con_dim)
    ...
    def forward(self, x,con_x, a):
        ddpmoutput = self.ddpm.sampling(x.shape[0],con_x, only_final=True)
        gnetoutput = self.gnet(x, a,ddpmoutput)
        return gnetoutput
    ...
    def loss_fn(self, x, con_x, lambda1, alpha, idx=None, train_stage="joint"):
        ...
        ddpmoutput = self.ddpm.sampling(x.shape[0],con_x, only_final=True)
        ddpm_loss = self.ddpm.loss_fn(x, con_x, alpha, idx=idx)
        quantile_loss = self.gnet.loss_fn(x, con_x, idx=idx)
        ...
        if train_stage == "quantile":
            ...
            y_pred = self.gnet(x, con_x,ddpmoutput)
            loss = self.gnet.loss_fn(y_pred, x)
            return lambda1*loss
