...
class DDPM(nn.Module):
    ...
    def _one_diffusion_step(self, x):
        ...
        for idx in reversed(range(len(self.alpha_bars))):
            ...
            predict_epsilon = self.backbone(x, idx1)
    ...
    def _one_diffusion_step_storerand(self, x):
        for idx in reversed(range(len(self.alpha_bars))):
            noise = torch.zeros_like(x) if idx == 0 else torch.randn_like(x)
            sqrt_tilde_beta = torch.sqrt((1 - self.alpha_prev_bars[idx]) / (1 - self.alpha_bars[idx]) * self.betas[idx])
            idx1 = torch.Tensor([idx for _ in range(x.size(0))]).to(device=self.device).long()
    ...
    @torch.no_grad()
    def sampling(self, sampling_number,only_final=False,ifreturnrand = False):
        '''
        ...
        if not ifreturnrand:
            ...
            for idx, (sample, noise) in enumerate(self._one_diffusion_step_storerand(sample)): # Unpack x and noise directly
                final = sample
        ...
        for epoch in range(num_epochs):
            ...
            with torch.no_grad():
                for val_batch in valdata_loader:
                    ...
                    val_loss += self.loss_fn(val_batch)
